...
#[test]
fn interval_map_iter_is_ok() {
    let mut map = IntervalMap::new();
    map.insert(Interval::new(1, 3), 1);
    map.insert(Interval::new(6, 7), 3);
    map.insert(Interval::new(2, 4), 2);

    for val in map.iter().filter(|&v| *v.1 == 2) {
        dbg!(&val);
    }
    let tmp: Vec<_> = map.iter().filter(|v| *v.0 == Interval::new(1, 3)).collect();
    dbg!(tmp);

    let tmp: Vec<_> = map.unsorted_iter().collect();
    dbg!(tmp);
}

#[test]
fn interval_map_into_iter_is_ok() {
    let mut map = IntervalMap::new();
    map.insert(Interval::new(1, 3), 1);
    map.insert(Interval::new(2, 4), 2);
    map.insert(Interval::new(6, 7), 3);
    ...
    let ttt: Vec<i32> = vec![1, 2, 3];
    let v: Vec<_> = ttt.iter().collect();
    dbg!(v);
    let v: Vec<_> = ttt.into_iter().collect();
    dbg!(v);
}
