...
fn check_overlaps_is_ok() {
    with_map_and_generator(|mut map, mut gen| {
        ...
        let expects: Vec<_> = to_check
            .iter()
            .map(|ci| intervals.iter().any(|i| ci.overlaps(i)))
            .collect();
...
fn find_all_overlaps_is_ok() {
    with_map_and_generator(|mut map, mut gen| {
        ...
        let expects: Vec<Vec<_>> = to_find
            .iter()
            .map(|ti| intervals.iter().filter(|i| ti.overlaps(i)).collect())
            .collect();
...
fn check_filter_iter_equal_to_iter_filter() {
    with_map_and_generator(|mut map, mut gen| {
        ...
        for i in intervals {
            ...
            let iter_filter_res: Vec<_> = map.iter().filter(|v| v.0.overlaps(&i)).collect();
