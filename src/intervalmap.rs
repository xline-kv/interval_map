...
use crate::index::{self, DefaultIx, IndexType, NodeIndex};
    ...
    Ix: IndexType,
{
    ...
    pub fn insert(&mut self, interval: Interval<T>, value: V) -> Option<V> {
        ...
        assert!(
            <Ix as IndexType>::max().index() == !0
                || <index::NodeIndex<_> as index::IndexType>::max() != node_idx,
    ...
    #[inline]
    #[must_use]
    pub fn into_iter(self) -> IntoIter<T, V, Ix> {
        IntoIter::new(self)
    }
    ...
    #[inline]
    pub fn contain(&self, interval: &Interval<T>) -> bool {
        let mut max_end: Option<&T> = None;
        let mut min_begin: Option<&T> = None;

        let mut continous = true;
...
mod test {
    ...
    fn test_interval_tree_intersects() {
        ...
        assert!(!map.overlap(&Interval::new_point("0")), "contains 0");
        assert!(map.overlap(&Interval::new_point("1")), "missing 1");
        assert!(map.overlap(&Interval::new_point("11")), "missing 11");
        assert!(map.overlap(&Interval::new_point("2")), "missing 2");
        assert!(!map.overlap(&Interval::new_point("3")), "contains 3");
    }
    ...
    struct TestCaseB {
        f: Box<dyn Fn(&(&Interval<i32>, &())) -> bool>,
        wcount: i32,
    }
    #[test]
    ...
    fn test_interval_tree_contains() {
        ...
        for (i, tt) in tests.iter().enumerate() {
            ...
            assert_eq!(map.contain(&tt.chk_ivl), tt.w_contains, "#{}: error", i);
        }
    }
    ...
    #[test]
    // todo: 后续实现比 iter().filter() 更快的 FilterIter
    fn test_interval_tree_sorted_visit() {
        ...
        for (i, tt) in tests.iter().enumerate() {
            let mut map = IntervalMap::new();
            let _ = tt.ivls.iter().for_each(|v| {
                map.insert(v.clone(), ());
            });
            let mut last = tt.ivls[0].low;
            let count = map
                .iter()
                .filter(|v| v.0.overlap(&tt.visit_range))
                .fold(0, |acc, v| {
                    assert!(
                        last <= v.0.low,
