...
impl<'a, T, V, Ix> Iterator for IntoIter<T, V, Ix>
where
    T: Ord,
    Ix: IndexType,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some((
            std::mem::replace(&mut res.interval.as_mut().unwrap(), unsafe {
                std::mem::zeroed()
            }),
            std::mem::replace(&mut res.value.as_mut().unwrap(), unsafe {
                std::mem::zeroed()
            }),
        ))
    }
